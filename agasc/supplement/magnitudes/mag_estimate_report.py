...
class MagEstimateReport:
    def __init__(
        ...
        if type(agasc_stats) is table.Table:
            ...
            if not Path(agasc_stats).exists:
                raise FileNotFoundError(
                    errno.ENOENT, os.strerror(errno.ENOENT), agasc_stats
                )
        ...
        if type(obs_stats) is table.Table:
            ...
            if not Path(obs_stats).exists:
                raise FileNotFoundError(
                    errno.ENOENT, os.strerror(errno.ENOENT), obs_stats
                )
    ...
    def single_star_html(
        ...
        agasc_stat['n_obs_bad'] = agasc_stat['n_obsids'] - agasc_stat['n_obsids_ok']
        agasc_stat['last_obs'] = ':'.join(
            obs_stat[-1]['mp_starcat_time'].split(':')[:4]
        )
    ...
    def multi_star_html(
        ...
        updated_star_ids = (
            ...
            if updated_stars is not None and len(updated_stars)
            else []
        )
        ...
        info = {
            'tstart': (
                tstart
                if tstart
                else CxoTime(self.obs_stats['mp_starcat_time']).min().date
            ),
            'tstop': (
                tstop
                if tstop
                else CxoTime(self.obs_stats['mp_starcat_time']).max().date
            ),
        ...
        if sections:
            agasc_ids = np.concatenate(
                [np.array(s['stars'], dtype=int) for s in sections]
            )
        else:
            agasc_ids = []
        ...
        fails = [
            {k: '' if v is None else v for k, v in f.items()}
        ...
        if len(agasc_stats):
            ...
            agasc_stats['n_obs_bad'] = (
                agasc_stats['n_obsids'] - agasc_stats['n_obsids_ok']
            )
            agasc_stats['flag'] = '          '
            agasc_stats['flag'][:] = ''
            agasc_stats['flag'][
        ...
        for agasc_id in tqdm(
            np.atleast_1d(agasc_ids).tolist(),
            desc='progress',
            disable=no_progress,
            unit='star',
        ):
            try:
                ...
                if make_single_reports:
                    self.single_star_html(
                        agasc_id, directory=dirname, highlight_obs=highlight_obs
                    )
        ...
        # this is a hack
        star_reports = {
            i: str(star_reports[i].relative_to(self.directory)) for i in star_reports
        }
        # make report
    ...
    def plot_agasc_id_single(
        ...
        if len(telem) == 0 or (
            arg_obsid is not None and np.sum(telem['obsid'] == arg_obsid) == 0
        ):
            msg = 'No Telemetry'
        ...
        for i, obsid in enumerate(obsids):
            ...
            if draw_obs_mag_stats and np.sum(sel):
                ...
                if np.isfinite(obs_stats[sel][0]['t_mean']) and np.isfinite(
                    obs_stats[sel][0]['t_std']
                ):
            ...
            if draw_roll_mean:
                ...
                roll_mean = mag_estimate.rolling_mean(
                    timeline['times'], timeline['mags'], window=100, selection=o
                )
                lh = ax.plot(
                    timeline['index'],
                    roll_mean,
        ...
        if (
            ...
            and np.isfinite(agasc_stat['mag_obs'])
            and agasc_stat['mag_obs'] > 0
        ):
    ...
    def plot_flags(telemetry, ax=None, obsid=None):
        ...
        if len(telemetry) > 0:
            ...
            timeline_dtype = np.dtype(
                ...
                + [
                    (n, int)
                    for n in ['AOACFCT', 'AOACASEQ', 'AOACIIR', 'AOPCADMD', 'x', 'y']
                ]
            )
        ...
        ax_dr = divider.append_axes("bottom", size='25%', pad=0.0, sharex=ax)
        if len(timeline) > 0:
            dr = timeline['dr'].copy()
            dr[dr > 10] = 10
            ax_dr.scatter(
            ...
            ax_dr.set_yticks([0.0, 2.5, 5, 7.5, 10], minor=True)
            ax_dr.set_yticks([0.0, 5, 10], minor=False)
...
GLOSSARY = {
    ...
    'Kalman samples': (
        'Subset of samples when ACA could be tracking stars (AOPCADMD == NPNT &'
        ' AOACASEQ == KALM)'
    ),
    'dr3': """
    ...
    'track': (
        'Subset of Kalman samples where the image is being tracked (AOACFCT == TRAK)'
    ),
    'sat_pix': 'Subset of Kalman samples with saturated pixel flag OK (AOACISP == OK)',
    'ion_rad': (
        'Subset of Kalman samples with ionizing radiation flag OK (AOACIIR == OK)'
    ),
    'mag_est_ok': (
