    ...
    def __init__(
        ...
        if exc_type is None:
            ...
            for step in traceback.extract_tb(exc_traceback):
                stack_trace.append(f"  in {step.filename}:{step.lineno}/{step.name}:")
...
def _magnitude_correction(time, mag_aca):
    ...
    """
    params = {
        "t_ref": "2011-01-01 12:00:00.000",
        "p": [
            0.005899340720522751,
            0.12029019332761458,
    ...
    dmag = q[0] + (q[1] + q[2] * np.atleast_1d(time)) * np.exp(
        q[3] + q[4] * np.atleast_1d(mag_aca)
    )
...
def get_telemetry(obs):
    ...
    slot_data = aca_l0.get_slot_data(
        start, stop, slot=obs['slot'], centered_8x8=True, columns=slot_data_cols
    )
    ...
    names = [
        ...
        f'AOACZAN{slot}',
        f'AOACYAN{slot}',
    ] + [f'AOATTQT{i}' for i in range(1, 5)]
    ...
    excluded_ranges = [
        (CxoTime(t[0]).cxcsec, CxoTime(t[1]).cxcsec) for t in excluded_ranges
    ]
    ...
    if len(slot_data) == 0:
        # the intersection was null.
        raise MagStatsException(
            'Nothing left after removing excluded ranges',
            agasc_id=obs["agasc_id"],
            obsid=obs["obsid"],
            mp_starcat_time=obs["mp_starcat_time"],
        )
    ...
    if np.any(mag_est_ok & (rang < 10)):
        ...
        telem['dr'] = (telem['dy'] ** 2 + telem['dz'] ** 2) ** 0.5
...
def get_mag_from_img(slot_data, t_start, ok=True):
    ...
    imgrow_8x8 = np.where(
        slot_data['IMGSIZE'] == 8, slot_data['IMGROW0'], slot_data['IMGROW0'] - 1
    )
    imgcol_8x8 = np.where(
        slot_data['IMGSIZE'] == 8, slot_data['IMGCOL0'], slot_data['IMGCOL0'] - 1
    )

    # subtract closest dark cal
    dark = np.zeros([len(slot_data), 8, 8], dtype=np.float64)
    staggered_aca_slice(
        dark_cal.astype(float), dark, 512 + imgrow_8x8, 512 + imgcol_8x8
    )
    ...
    col = np.ma.sum(pixel_center * projected_image, axis=1) / np.ma.sum(
        projected_image, axis=1
    )
    projected_image = np.ma.sum(slot_data['IMGRAW'], axis=2)
    row = np.ma.sum(pixel_center * projected_image, axis=1) / np.ma.sum(
        projected_image, axis=1
    )
...
OBS_STATS_INFO = {
    ...
    'f_kalman': (
        'Fraction of all samples where AOACASEQ == "KALM" and AOPCADMD == "NPNT"'
        ' (n_kalman/n)'
    ),
    ...
    'obs_fail': (
        'Boolean flag: a processing error when estimating magnitude for this'
        ' observation'
    ),
    'comments': '',
...
def get_obs_stats(obs, telem=None):
    ...
    stats = {
        k: obs[k] for k in ['agasc_id', 'obsid', 'slot', 'type', 'mp_starcat_time']
    }
...
def calc_obs_stats(telem):
    ...
    aoacmag_q25, aoacmag_q50, aoacmag_q75 = np.quantile(
...
AGASC_ID_STATS_INFO = {
    ...
    'f_mag_est_ok_3': (
        ...
        ' 3 arcsec (n_mag_est_ok_3 / n_kalman)'
    ),
    'f_mag_est_ok_5': (
        'Fraction of kalman samples that are included in magnitude estimate and within'
        ' a 5 arcsec box (n_mag_est_ok_5 / n_kalman)'
    ),
    ...
    'n_dbox5': (
        'Number of mag-est-ok samples with centroid residual within a 5 arcsec box'
    ),
    'n_dbox5_outliers': (
        'Number of magnitude outliers after removing outliers and samples with '
        'centroid residual out of a 5 arcsec box, on a per-observation basis'
...
def get_agasc_id_stats(agasc_id, obs_status_override=None, tstop=None):
    ...
    result.update(
        {
            ...
            'n_no_mag': np.count_nonzero((~stats['obs_ok'])) + np.count_nonzero(
                stats['f_mag_est_ok'][stats['obs_ok']] < 0.3
            ),
            'n': n_total,
            'n_mag_est_ok': np.count_nonzero(mag_est_ok),
    ...
    for dr in [3, 5]:
        ...
        if not np.any(k):
            continue
        sigma_minus, q25, median, q75, sigma_plus = np.quantile(
            mags[k], [0.158, 0.25, 0.5, 0.75, 0.842]
        )
        outlier = mag_est_ok & all_telem['obs_outlier']
        mag_not = (
            np.nanmean(mags[k2 & (~outlier)])
            if np.count_nonzero(k2 & (~outlier))
            else np.nan
        )
        std_not = (
            np.nanstd(mags[k2 & (~outlier)])
            if np.count_nonzero(k2 & (~outlier))
            else np.nan
        )
        result.update(
    ...
    result.update(
        {
            ...
            'selected_rtol': (
                np.abs(result['mag_obs'] - result['mag_aca'])
                > 3 * result['mag_aca_err']
            ),
            'selected_mag_aca_err': result['mag_aca_err'] > 0.2,
            'selected_color': (result['color'] == 1.5) | (
                np.isclose(result['color'], 0.7)
            ),
        }
    )
